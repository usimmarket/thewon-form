...
const WRAP_CONFIG = {
  subscriber_name:        { maxWidth: 320, lineHeight: 22, maxLines: 2, charLimit: 25 },
  autopay_holder:         { maxWidth: 320, lineHeight: 22, maxLines: 2, charLimit: 25 },
  subscriber_name_print:  { maxWidth: 320, lineHeight: 22, maxLines: 2, charLimit: 25 },
  autopay_holder_print:   { maxWidth: 320, lineHeight: 22, maxLines: 2, charLimit: 25 }
};
...
function splitByCharLimit(text, limit, maxLines) {
  const s = String(text || '');
  const n = Math.max(1, Number(limit) || 25);
  const m = Math.max(1, Number(maxLines) || 2);
  const out = [];
  for (let i = 0; i < s.length && out.length < m; i += n) {
...
function drawWrapped2Lines(page, font, text, x, y, size, key) {
  ...
  const s = String(text || '');
  const lines = (cfg.charLimit && s.length > cfg.charLimit)
    ? splitByCharLimit(s, cfg.charLimit, cfg.maxLines || 2)
    : layoutWrappedNoEllipsis(s, font, size, maxWidthPt, cfg.maxLines || 2);
