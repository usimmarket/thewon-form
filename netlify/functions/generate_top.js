...
function splitByCharLimitTwoLines(text, limit) {
  const s = String(text ?? '');
  if (!limit || s.length <= limit) return [s];
  // Prefer breaking at the last space before the limit
  const head = s.slice(0, limit);
  const lastSpace = head.lastIndexOf(' ');
  const cut = (lastSpace >= Math.max(3, limit - 10)) ? lastSpace : limit; // avoid weird early breaks
  const line1 = s.slice(0, cut).trimEnd();
  const line2 = s.slice(cut).trimStart();
  return [line1, line2];
}
...
exports.handler = async (event) => {
  ...
  // AUTOFILL_AUTOPAY_HOLDER_FROM_SUBSCRIBER: keep behavior consistent with FreeT
  if ((!data.autopay_holder || String(data.autopay_holder).trim() === '') && data.subscriber_name) {
    data.autopay_holder = data.subscriber_name;
  }
